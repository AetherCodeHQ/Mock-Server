...
> Mock-Server - Open source tool by AetherCodeHQ

`api` `cli` `developer-tools` `golang` `mocking` `open-source` `testing`

## What is it?

**Mock Server** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Mock-Server --path ./target

# With options
./Mock-Server --path ./target --format json --output report.json

# Verbose mode
./Mock-Server --path ./target --verbose
```
...
### Basic scan
```bash
./Mock-Server --path ./my-project
```

### JSON report
```bash
./Mock-Server --path ./my-project --format json --output report.json
```
...
- name: Run Mock Server
  run: ./Mock-Server --path . --format json --output report.json
```
